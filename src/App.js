...
import { FaShoppingCart, FaUserAlt, FaSearch, FaLeaf, FaBreadSlice, FaCheese, FaAppleAlt, FaCarrot, FaCheckCircle } from "react-icons/fa";
import "./App.css";
    ...
    <header className="header-modern">
      ...
      <nav className="nav-modern">
        ...
        <div
          ...
          onMouseEnter={() => { setShowMegaMenu(true); setHoveredCategory("Snacks & Beverages"); }}
          onMouseLeave={() => { setShowMegaMenu(false); setHoveredCategory(null); }}
        >
          <a href="#" onClick={e => e.preventDefault()} style={{ fontWeight: 600 }}>Snacks & Beverages ▾</a>
          {showMegaMenu && hoveredCategory === "Snacks & Beverages" && (
            <div className="mega-menu-dropdown">
              <div className="mega-menu-columns">
                {megaMenuCategories.map(cat => (
                  <div key={cat.name} className="mega-menu-category-col">
                    <div className="mega-menu-category-title">{cat.name}</div>
                    {cat.subcategories && cat.subcategories.map(subcat => (
                      <div
                        key={subcat}
                        className="mega-menu-subcat"
                        onClick={() => { handleCategoryNavClick(cat.name); handleSubcategoryNavClick(subcat); setShowMegaMenu(false); }}
                      >
                        {subcat}
                      </div>
                    ))}
...
function Footer() {
  return (
    <footer className="footer-modern">
      <div>© 2025 Yral Foods. All rights reserved.</div>
...
function App() {
  ...
  return (
    <div className="app-modern">
      ...
      />
      <Footer />
      <LoginModal open={loginOpen} onClose={() => setLoginOpen(false)} onLoginSuccess={() => { setIsLoggedIn(true); setLoginOpen(false); }} />
      <CartDrawer 
        open={cartOpen} 
